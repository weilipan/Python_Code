...
教學製作：建國高中資訊科潘威歷老師
製作日期：2021/06/07
第十一單元：圖形轉換方法convert,convert_alpha
'''
...
class Background(object):
    def __init__(self):
        self.X = 0 #背景圖案的預設X座標
        self.Y = 0 #背景圖案的預設Y座標
        self.img=pygame.image.load('./img/bg_day.png').convert() #背景圖案的Surface物件,載入後加上convert()轉換為專屬格式，可加快執行速度。
...
class Bird(object):
    def __init__(self):
        # 有關Bird的相關圖檔集，建立圖案陣列
        self.birdimages=[
            pygame.image.load('./img/bird0_0.png').convert_alpha(), #若有透過背景圖，則要用convert_alpha()
            pygame.image.load('./img/bird0_1.png').convert_alpha(),
            pygame.image.load('./img/bird0_2.png').convert_alpha(),
            pygame.image.load('./img/dead.png').convert_alpha()
        ]
...
class Pipe(object):
    def __init__(self):
        ...
        self.pipeUp = pygame.image.load('./img/pipe_up.png').convert_alpha() #由下方往上延伸的水管圖形
        self.pipeDown = pygame.image.load('./img/pipe_down.png').convert_alpha() #由上方往下延伸的水管圖形
